...
// La création des comptes organisateurs est réservée à un administrateur déjà authentifié.
router.post(
  "/register",
  requireAuth,
  requireRole("admin"),
  ...
  async (req, res, next) => {
    try {
      ...
      const user = await queryOne(
        "INSERT INTO users (name, email, password_hash, role) VALUES ($1,$2,$3,'editeur') RETURNING id, name, email, role",
        [name, email, hash]
      );
