...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (Number(team_id) !== Number(match.home_team_id) && Number(team_id) !== Number(match.away_team_id)) {
        return res.status(400).json({ error: "L'équipe de l'événement ne participe pas à ce match." });
      }
      if (player_id) {
        const player = await queryOne("SELECT id, team_id FROM players WHERE id = $1", [player_id]);
        if (!player) return res.status(404).json({ error: "Joueur introuvable." });
        if (Number(player.team_id) !== Number(team_id)) {
          return res.status(400).json({ error: "Le joueur n'appartient pas à l'équipe sélectionnée." });
        }
      }
      if (minute !== undefined && minute !== null && (!Number.isInteger(Number(minute)) || Number(minute) < 0 || Number(minute) > 130)) {
        return res.status(400).json({ error: "Minute invalide (0 à 130)." });
      }
